...
export function SingleImageInterceptor(fieldName: string, dest = UPLOAD_CONSTANTS.IMAGE_DEST): Type<any> {
    return FileInterceptor(fieldName, buildMulterOptions('image', dest));
}

export function MultipleImagesInterceptor(fieldName: string, maxCount = UPLOAD_CONSTANTS.MAX_IMAGES_COUNT, dest = UPLOAD_CONSTANTS.IMAGE_DEST): Type<any> {
    return FilesInterceptor(fieldName, maxCount, buildMulterOptions('image', dest));
}

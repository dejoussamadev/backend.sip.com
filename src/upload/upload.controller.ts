...
export class UploadController {
  ...
  uploadSingleImage(
    ...
    return {
      ...
      data: this.uploadService.toFileInfo(file, baseUrl),
    };
  }
  ...
  uploadMultipleImages(
    ...
    if (!files?.length)
      return { success: false, message: 'Aucune image reçue', data: [] };
    const baseUrl = `${req.protocol}://${req.get('host')}`;
    return {
      success: true,
      message: `${files.length} image(s) uploadée(s)`,
      data: this.uploadService.toFilesInfo(files, baseUrl),
    };
  }
  ...
  uploadPropertyFiles(
    ...
    const baseUrl = `${req.protocol}://${req.get('host')}`;
    const imageInfos = this.uploadService.toFilesInfo(
      files.images ?? [],
      baseUrl,
    );
    const documentInfos = this.uploadService.toFilesInfo(
      files.documents ?? [],
      baseUrl,
    );
    return {
